...
export default function DaySchedule({ date, onEditGoal, showForm = true }) {
  ...
  const handleEdit = onEditGoal || setEditingGoal;
  ...
  return (
    <section className={showForm ? "grid gap-5 lg:grid-cols-[20rem_1fr]" : "grid gap-5"}>
      {showForm && <GoalBlockForm date={activeDate} editingGoal={editingGoal} onDone={() => setEditingGoal(null)} />}
      ...
      <div className="rounded-[2rem] bg-white/70 p-5 shadow-xl shadow-zinc-200/60 backdrop-blur">
        ...
        <div className="mt-5">
          {hours.map((hour) => (
            <TimeBlock hour={hour} key={hour} onEdit={handleEdit} scheduledGoals={dayGoals} />
          ))}
        </div>
      </div>
    </section>
