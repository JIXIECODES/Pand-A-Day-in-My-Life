...
export function GoalComposer({ kind = "daily", onSaved }) {
  const {
    addClassicGoal,
    addLongTermGoal,
  } = useAppContext();
  ...
  return (
    <form className="rounded-3xl bg-zinc-50 p-4" onSubmit={submit}>
...
export default function HomeGoals({ kind = "daily", showComposer = true }) {
  const {
    classicGoals,
    longTermGoals,
    scheduledGoals,
  ...
  const config = sectionConfig[kind] || sectionConfig.daily;
  const scheduledForKind = scheduledGoals
    .filter((goal) => (kind === "longTerm" ? goal.goalType === "long-term" : goal.goalType === "daily"))
    .map((goal) => ({ ...goal, linkedLabel: "Calendar block" }));
  const goals = kind === "longTerm"
    ? [...scheduledForKind, ...longTermGoals]
    : [...scheduledForKind, ...classicGoals];
  ...
  return (
    <section className="rounded-[2rem] bg-white/75 p-5 shadow-xl shadow-zinc-200/60">
      ...
      {showComposer && showForm && <div className="mb-4"><GoalComposer kind={kind} onSaved={() => setShowForm(false)} /></div>}
