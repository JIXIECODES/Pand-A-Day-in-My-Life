...
export default function CalendarPage() {
  ...
  const [classicGoalsOpen, setClassicGoalsOpen] = useState(false);
    ...
    <main className="mx-auto grid max-w-7xl gap-5 px-4 py-6 sm:px-6">
      ...
      <section className="rounded-[2rem] bg-white/70 p-5 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button className="rounded-full bg-zinc-950 px-5 py-3 font-black text-white" onClick={() => setClassicGoalsOpen(true)} type="button">
            Open daily goals
          </button>
        </div>
      </section>
      <DayPlannerModal open={modalOpen} onClose={() => setModalOpen(false)} />
      <GoalModal open={classicGoalsOpen} onClose={() => setClassicGoalsOpen(false)} />
    </main>
  );
}
